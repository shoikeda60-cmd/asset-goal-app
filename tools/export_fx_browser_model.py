...
def predict_exported_hgb(params,x):
    ...
    for nodes in params['trees']:
        ...
        while not nodes[idx]['leaf']:
            ...
            if not np.isfinite(val): idx=n['l'] if n['ml'] else n['r']
            else: idx=n['l'] if val<=n['t'] else n['r']
        raw+=nodes[idx]['v']
...
def main():
    m1,rows=bt.build()
    # Architecture/thresholds were selected without using Jul-Aug holdout.
    train=[r for r in rows if r['t']<bt.HOLD]
    ...
    # Parity check exported tree evaluator vs sklearn on a sample.
    sample=Xd[:min(200,len(Xd))]
    skl=direction.predict_proba(sample)[:,1]
    exp=np.array([predict_exported_hgb(hp,x) for x in sample])
    max_err=float(np.max(np.abs(skl-exp))) if len(sample) else 0
    result={
      'version':1,
      'created_for':'asset-goal-app FX analysis',
      'target_pips_reference':bt.TARGET,
      'feature_count':len(train[0]['x']),
      'thresholds':{'reach':REACH_THRESHOLD,'direction':DIR_THRESHOLD},
      'reach':log_params(reach),
      'direction':hp,
      'validation':metric(reach,direction,holdout),
      'validation_period':'2026-07-01 through 2026-08-14 (untouched holdout)',
      'tree_export_max_probability_error':max_err,
      'training_note':'Production model refit on Apr-Jun after architecture/threshold selection; Jul-Aug remains holdout.'
    }
    with open('fx-model.json','w',encoding='utf-8') as f: json.dump(result,f,separators=(',',':'))
    print('BROWSER_MODEL_EXPORT_RESULT')
    print(json.dumps({'validation':result['validation'],'tree_export_max_probability_error':max_err,'model_bytes':__import__('os').path.getsize('fx-model.json')},indent=2))
